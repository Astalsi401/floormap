...
import store, { setSearchCondition, setElementStatus, useAppSelector, useAppDispatch } from "@store";
...
export const Header: React.FC = () => {
  ...
  const tags = [mapText.event, ...(mapText.headerTags || [])].filter((d) => d !== undefined);
  return (
    <div className="fp-tags px-2 py-1 shadow">
      <div className="d-flex">
        ...
        <div className="fp-download flex-shrink-0" title={mapText.download} onClick={() => download({ floor })}>
      ...
      <div className="py-1 gap-1 d-flex flex-wrap align-items-center">
        ...
        {tags.map((tag) => (
          <div
            ...
            style={{ "--cat": colors.scale(tag) } as React.CSSProperties}
            onClick={() => {
...
const download = async ({ floor }: { floor: string }) => {
  await new Promise((resolve) => setTimeout(resolve, 50));
  const { width, height } = store.getState().elementStatus;
  const svg: SVGSVGElement | null = document.querySelector("svg#floormap");
  if (!svg) return;
  resetViewbox({ svg });
